...
REQUIRE $!  ~mak/place.f
REQUIRE CASE Meta_x86_64/SRC/case.4 
...
\  GRENAME IF TYPE BREAK
 2DUP S" DUP" UCOMPARE 0= IF 2DROP ." _DUP" BREAK
 2DUP S" ABS" UCOMPARE 0= IF 2DROP ." _ABS" BREAK
 2DUP S" ALIGN" UCOMPARE 0= IF 2DROP ." _ALIGN" BREAK
 2DUP S" WORD" UCOMPARE 0= IF 2DROP ." _WORD" BREAK
 2DUP S" TO" UCOMPARE 0= IF 2DROP ." _TO" BREAK

 OVER C@ '0' - 10 U< IF ." _" THEN
  0 DO  MCOUNT
      CASE
      [CHAR] @ OF ." load" ENDOF
      [CHAR] ! OF ." save" ENDOF
...
\             00 00 T, .NOOP 0F???"
 ?REX. DROP 1- TAB ." DB	0x"  ?REX.X 'F' EMIT
...
\  ['] .ESC	TO INST.	D8 T" f"
  ...
  DROP TAB ." DB	0x" ?REX.X OPCODE C@ H.-
;
...
\  TR_FLG IF DUP ." ( "  HEX. ." ) "  DUP @ H.  CR .S CR KEY DROP  THEN
  ...
  DUP ROT TAB ." ; " DUP H.
  //WAS
  IF CR TAB  ." DB 0x"  COUNT DUP H.-
	$BB = \ drop 0
	IF
 DUP  CR TAB   ." DD " L@ .$DS 4 +
 2DUP <> IF
...
\  TR_FLG IF DUP CR ." ( "  HEX. ." ) "  DUP @ H.  CR .S CR KEY DROP  THEN
  ...
  THEN
  DOVECT?
  IF   CR TAB ." DQ "  DUP @ ?.NAME>S 8 +	0 TO DOVECT?
  THEN
